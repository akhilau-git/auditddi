...
AuditDDI's unseen-drug (S1) performance. It is a research candidate, not a
backend deployment procedure.

## Evidence available

The audited ChEMBL 37 `chemreps` source contains 2,897,804 valid canonical
SMILES records. It has exact structural overlap with 414 of 645 valid AuditDDI
...
Before selection, the pretraining script recreates the AuditDDI standard split
...
%env AUDITDDI_DATA_CAP=200000
%env AUDITDDI_SPLIT_SEED=42
%env AUDITDDI_NEGATIVE_SAMPLING_STRATEGY=degree_matched
%env AUDITDDI_NEGATIVE_SAMPLING_PROTOCOL=split_aware_standard_v1
%env AUDITDDI_HIDDEN_CHANNELS=128
```
...
```python
%env AUDITDDI_PRETRAIN_SEED=2026
%env AUDITDDI_PRETRAIN_MAX_MOLECULES=50000
%env AUDITDDI_PRETRAIN_EPOCHS=20
%env AUDITDDI_PRETRAIN_BATCH_SIZE=256
...
`auditddi-results/pretraining/...` folder. Inspect its
...
```python
%env AUDITDDI_CHEMBL_PRETRAINED_ENCODER_PATH=PASTE_THE_PRINTED_ENCODER_CHECKPOINT_PATH
%env AUDITDDI_EXPERIMENTS_BASE=/content/drive/MyDrive/auditddi-results/experiments
%env AUDITDDI_EXPERIMENT_PRESET=screening
%env AUDITDDI_EXPERIMENT_SEEDS=42
%env AUDITDDI_EXPERIMENT_SPLIT_SEED=42
%env AUDITDDI_EXPERIMENT_NEGATIVE_SAMPLING_PROTOCOL=split_aware_standard_v1
%env AUDITDDI_EXPERIMENT_EPOCHS=200
%env AUDITDDI_EXPERIMENT_NAMES=edge_aware_multitask,edge_aware_chembl_pretrained_multitask
%env AUDITDDI_EXPERIMENT_REFERENCE=edge_aware_multitask
...
```python
%env AUDITDDI_EXPERIMENT_PRESET=paper
%env AUDITDDI_EXPERIMENT_SEEDS=11,23,37,53,71
%env AUDITDDI_EXPERIMENT_SPLIT_SEED=42
%env AUDITDDI_EXPERIMENT_EPOCHS=200
%env AUDITDDI_EXPERIMENT_NAMES=edge_aware_multitask,edge_aware_chembl_pretrained_multitask
%env AUDITDDI_EXPERIMENT_REFERENCE=edge_aware_multitask
